...
var packr = new import_msgpackr.Packr({ useRecords: false, maxSharedStructures: 8e3 });
async function encodeData(data) {
  ...
  const compressed = await compress(packed, {
    params: {
      ...
      [import_zlib.default.constants.BROTLI_PARAM_MODE]: import_zlib.default.constants.BROTLI_MODE_GENERIC
    }
  });
  return {
    payload: compressed,
